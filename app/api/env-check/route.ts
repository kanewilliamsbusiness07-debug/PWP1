...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    summary: {
      ...
      isAmplify: Boolean(process.env.AMPLIFY_APP_ID || process.env.CODEBUILD_BUILD_ID),
    ...
    troubleshooting: {
      issue: process.env.DATABASE_URL ? 'Environment variables appear to be set' : 'DATABASE_URL is missing',
      steps: [
        '1. Go to AWS Amplify Console → Your App → App settings → Environment variables',
        '2. Click "Manage variables" or "Add variable"',
        '3. Ensure you are setting variables for the correct branch/environment',
        '4. For branch "fix-amplify-deploy", you may need to:',
        '   - Set variables specifically for that branch, OR',
        '   - Use "Apply to all branches" option',
        '5. After adding variables, click "Save"',
        '6. Go to Deployments → Click "Redeploy this version" on latest deployment',
        '7. Wait for deployment to complete',
        '8. Check this endpoint again to verify variables are loaded',
      ],
      note: 'Environment variables are only loaded when a new deployment starts. You MUST redeploy after adding/updating variables.'
    }
  });
}
